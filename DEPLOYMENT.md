...
Le seguenti risorse sono state configurate nel progetto GCP `credem-aiddat-workarea-coll`:
...
3.  **Service Account:** È stato identificato un service account esistente con i permessi necessari (`roles/ml.admin`) per l'applicazione.
    *   **Service Account Selezionato:** `737921688154-compute@developer.gserviceaccount.com`
...
```bash
docker build -t europe-west1-docker.pkg.dev/credem-aiddat-workarea-coll/checklist-agent-repo/checklist-agent:latest .
```
...
```bash
docker push europe-west1-docker.pkg.dev/credem-aiddat-workarea-coll/checklist-agent-repo/checklist-agent:latest
```
...
gcloud run deploy checklist-agent \
  --image=europe-west1-docker.pkg.dev/credem-aiddat-workarea-coll/checklist-agent-repo/checklist-agent:latest \
  --region=europe-west1 \
  --service-account=737921688154-compute@developer.gserviceaccount.com \
  --allow-unauthenticated \
  --set-env-vars="AUTH_MODE=ADC" \
  --set-env-vars="GCP_PROJECT_ID=credem-aiddat-workarea-coll" \
